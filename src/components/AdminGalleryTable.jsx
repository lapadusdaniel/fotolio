...
function GalleryRow({
  ...
  const galleryUrl = galerie?.slug
    ? `${window.location.origin}/${galerie.slug}`
    : galerie?.id ? `${window.location.origin}/gallery/${galerie.id}` : ''
  ...
  const handleShare = (e) => {
    e.stopPropagation()
    if (galleryUrl && navigator.share) {
      navigator.share({ url: galleryUrl, title: galerie?.nume || 'Galerie' }).catch(() => {})
    } else if (galleryUrl) {
...
export default function AdminGalleryTable({
  ...
  const [pinnedGaleriiIds, setPinnedGaleriiIds] = useState(() => {
    try {
  ...
  const handleArchive = async (id) => {
    try {
      await updateDoc(doc(db, 'galerii', id), { status: 'archived' })

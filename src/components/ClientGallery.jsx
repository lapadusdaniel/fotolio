...
const ClientGallery = () => {
    ...
    const fetchDate = async () => {
      try {
        ...
        if (galleryId) {
          ...
          if (!docSnap.exists()) { setEroare('Galeria nu a fost găsită.'); setLoading(false); return; }
          dateGal = { id: docSnap.id, ...docSnap.data() };
        } else {
          const q = query(collection(db, 'galerii'), where('slug', '==', slug));
          const querySnapshot = await getDocs(q);
          if (querySnapshot.empty) { setEroare('Galeria nu a fost găsită.'); setLoading(false); return; }
        ...
        if (dateGal.userId) {
          try {
            ...
            if (snap.exists()) {
              ...
              if (d.logoUrl) { try { logoPreviewUrl = await getBrandingUrl(d.logoUrl); } catch { } }
        ...
        if (pozeKeys[0]) {
          ...
          getPozaUrl(coverKey, 'thumb').then((url) => { setCoverThumbUrl(url); urlCache.set(`thumb:${coverKey}`, url); }).catch(() => {});
        }
      } catch (err) {
  ...
  useEffect(() => {
    ...
    getPozaUrl(coverKey, 'original').then((url) => { urlCache.set(`original:${coverKey}`, url); setCoverOriginalUrl(url); }).catch(() => {});
  }, [poze]);
  ...
  const handleSaveName = async () => {
    ...
    if (!galerie?.numeSelectieClient && titleToSave) {
      try { await updateDoc(doc(db, 'galerii', galerie.id), { numeSelectieClient: titleToSave }); setGalerie(prev => ({ ...prev, numeSelectieClient: titleToSave })); } catch (e) { console.error(e); }
    }
  ...
  const executeFavoriteToggle = async (pozaKey, numeClient) => {
    ...
    try {
      if (isFav) {
        ...
        await updateDoc(galerieRef, { favorite: arrayUnion(pozaKey), [`selectii.${numeClient}`]: arrayUnion(pozaKey) });

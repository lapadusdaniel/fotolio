  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      for (const g of galleries.slice(0, 3)) {
        try {
          const poze = await listPoze(g.id, g.userId)
...
export default function PhotographerSite({ previewData = null }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        // Caută fotograful după slug (generat din brandName)
        const q = query(
        ...
        // Încarcă profilul
        const profileSnap = await getDoc(doc(db, 'profiles', data.uid))
        if (profileSnap.exists()) setProfile(profileSnap.data())

        // Încarcă galeriile selectate
        if (data.portfolioGalleryIds?.length) {
          const gSnaps = await Promise.all(
            data.portfolioGalleryIds.map(id => getDoc(doc(db, 'galerii', id)))
          )
          const gs = gSnaps
            .filter(s => s.exists())
  ...
  useEffect(() => {
    ...
    if (siteData.logoPath) {
      getBrandingUrl(siteData.logoPath).then(setLogoUrl).catch(() => {})
    }
    if (siteData.heroImagePath) {
      getPozaUrl(siteData.heroImagePath, 'original').then(setHeroImageUrl).catch(() => {})
    }
    if (siteData.aboutImagePath) {
      getPozaUrl(siteData.aboutImagePath, 'original').then(setAboutImageUrl).catch(() => {})
    }
  }, [siteData])
  ...
  if (loading) {
    return (
      <div className="ps-loading">
        <p className="ps-loading-logo">Fotolio</p>
    ...
    <div className="ps-root">
      ...
      <footer className="ps-footer">
        ...
        <span className="ps-footer-fotolio">
          Creat cu <a href="https://fotolio.app" target="_blank" rel="noreferrer">Fotolio</a>
        </span>
      </footer>

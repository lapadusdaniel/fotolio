...
export default function SiteEditor({ user, userGalleries = [] }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const bn = profileSnap.exists() ? (profileSnap.data().brandName || '') : ''
        setBrandName(bn)
        const sl = generateSlug(bn)
        setSlug(sl)

        // Ia datele de site
        const siteSnap = await getDoc(doc(db, 'photographerSites', user.uid))
        if (siteSnap.exists()) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await setDoc(doc(db, 'photographerSites', user.uid), {
        ...form,
        uid: user.uid,
        brandName,
        slug: sl,

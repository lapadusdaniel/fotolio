...
import {
  ...
  query, orderBy, where, onSnapshot, setDoc
} from 'firebase/firestore'
import {
  LayoutDashboard, Users, Images, CreditCard, MessageSquare,
  HardDrive, Settings, LogOut, X, ChevronRight, Search,
  AlertTriangle, CheckCircle, Ban, Trash2, Edit3, RefreshCw,
  TrendingUp, Eye
} from 'lucide-react'
import './AdminPanel.css'

// ── Constante ─────────────────────────────────
const ADMIN_UID = 'SmV217ylQdWmLaRQcclUTIfwKC12'
...
const STORAGE_LIMITS = { Free: 15, Pro: 500, Unlimited: 1000 }
...
function UsersSection({ users, loading, onChangePlan, onToggleSuspend, onDelete }) {
                ...
                {filtered.map(g => (
                  <tr key={g.id}>
                    ...
                    <td>
                      <div className="ap-row-actions">
                        {g.slug && (
                          <a href={`/${g.slug}`} target="_blank" rel="noreferrer" className="ap-row-btn" title="Deschide">
                            <Eye size={14} />
                          </a>
                        )}
                      </div>
...
function SetariSection() {
  ...
  useEffect(() => {
    getDoc(doc(db, 'adminSettings', 'platform')).then(snap => {
  ...
  const handleSave = async () => {
    setSaving(true)
    try {
      await setDoc(doc(db, 'adminSettings', 'platform'), { ...settings, updatedAt: new Date() }, { merge: true })
  ...
  return (
    <div>
      <div className="ap-page-header">
        ...
        <p className="ap-page-sub">Configurări globale Fotolio</p>
      </div>
...
export default function AdminPanel({ user }) {
    ...
    const load = async () => {
      try {
        ...
        for (const userDoc of usersSnap.docs) {
          ...
          // Numără galeriile
          const galeriiSnap = await getDocs(query(collection(db, 'galerii'), where('userId', '==', uid)))
          const galeriiCount = galeriiSnap.size

          // Citește planul manual (dacă există override)
          const overrideDoc = await getDoc(doc(db, 'adminOverrides', uid))
          const planOverride = overrideDoc.exists() ? overrideDoc.data().plan : null
  ...
  // ── Load galerii ──
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, 'galerii'), (snap) => {
      setGalerii(snap.docs.map(d => ({ id: d.id, ...d.data() })))
  ...
  const handleChangePlan = async (uid, newPlan) => {
    try {
      await setDoc(doc(db, 'adminOverrides', uid), { plan: newPlan, updatedAt: new Date() }, { merge: true })
  ...
  const handleToggleSuspend = async (targetUser) => {
    ...
    try {
      ...
      await setDoc(doc(db, 'profiles', targetUser.uid), { status: newStatus }, { merge: true })
  ...
  const handleDeleteUser = async (targetUser) => {
    ...
    try {
      ...
      try { await deleteDoc(doc(db, 'profiles', targetUser.uid)) } catch {}
    ...
    <div className="ap-root">
      <header className="ap-topbar">
        <div className="ap-topbar-left">
          <a href="/dashboard" className="ap-logo">Fotolio</a>
